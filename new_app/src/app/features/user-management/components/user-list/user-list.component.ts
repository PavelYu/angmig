...
export class UserListComponent implements OnInit {
  ...
  onGridReady(event: GridReadyEvent): void {
    ...
    // Restore grid state using GridColumnApi (AG Grid v28 API)
    const savedState = this.gridStateService.getGridState('user-list');
    if (savedState && this.gridColumnApi && savedState.columnState) {
      this.gridColumnApi.applyColumnState({
        state: savedState.columnState,
        applyOrder: true
      });
    }
    if (savedState && this.gridColumnApi && savedState.columnGroupState) {
      this.gridColumnApi.applyColumnState({
        state: savedState.columnState,
        applyOrder: true
      });
  ...
  onToggleColumns(): void {
    ...
    // Note: AG Grid v28 SideBar API changed - use setSideBarVisible instead
