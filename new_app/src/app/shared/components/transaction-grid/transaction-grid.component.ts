...
import { ColDef, GridApi, ColumnApi, GridOptions, GridReadyEvent, IServerSideGetRowsParams } from 'ag-grid-community';
// Note: ServerSideRowModelModule is not exported from ag-grid-community in v28
// Server-side row model is configured via gridOptions.rowModelType
...
export class TransactionGridComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // Grid state will be restored in onGridReady using GridApi
    // Note: columnState/columnGroupState don't exist on GridOptions in AG Grid v28+
  }
  ...
  onToggleColumns(): void {
    // AG Grid v28 SideBar API changed - use setSideBarVisible instead
    const isVisible = this.gridApi.isSideBarVisible();
    this.gridApi.setSideBarVisible(!isVisible);
  }
  ...
  saveGridState(): void {
    if (this.gridColumnApi) {
      ...
      this.gridStateService.saveGridState(this.gridId, {
        columnState,
        columnGroupState: {} // Note: columnGroupState API may differ in AG Grid v28
      });
    }
  }
