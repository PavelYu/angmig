...
> - **[../setup/zed-mcp-setup.md](../setup/zed-mcp-setup.md)** ← **Setup first!** Zed + MCP for 5x faster migration
> - **[quick-reference.md](quick-reference.md)** ← Day-by-day checklist with AI prompts
> - **[parallelization.md](parallelization.md)** ← Complete parallelization strategy
...
**Impact**: 5x faster migration with higher quality. See **[../setup/zed-mcp-setup.md](../setup/zed-mcp-setup.md)** for setup.
...
1.  **Install Playwright** (Non-Interactive Method):
    ```bash
    # ✅ CORRECT: Non-interactive installation
    ...
    > [!NOTE]
    > **Pattern Learned**: `npm init playwright@latest --yes` flag doesn't exist. Use `npm install` for non-interactive setup.
...
> [!CRITICAL]
> **BULLETPROOF PREREQUISITES** - These checks MUST pass before starting Phase 1:
> 1. ✅ **Phase 0 Complete** - Playwright baseline captured, dependency audit started
> 2. ✅ **Version Verification** - Actual Angular version confirmed
> 3. ✅ **Build Verification** - Current build passes
> 4. ✅ **Git Clean** - Repository ready for migration
...
#### Prerequisites (MANDATORY - All Must Pass)
> [!CRITICAL]
> **BULLETPROOF CHECKLIST** - Verify ALL before starting Phase 2:

- [ ] **Version Verification** ✅
  ```bash
  npm list @angular/core --depth=0
  # Verify actual version - may be v14, not v15!
  ```
  > [!WARNING]
  > **Pattern Learned**: Always verify actual Angular version first. Plan may assume v15, but app could be v14.3.0.

- [ ] **Git Repository Clean** ✅
  ```bash
  git status
  # Must be clean OR changes committed/stashed
  ```
  > [!WARNING]
  > **Pattern Learned**: `ng update` requires clean git repository. Stash or commit changes first.
...
- [ ] **Node.js Version** ✅
  ...
  ```
  > [!WARNING]
  > **Pattern Learned**: Node.js version must match Angular requirements. v20 may cause issues with Angular 14-15 CLI.
...
```bash
# ⚠️ CRITICAL: Ensure git repository is clean first!
git status  # Must be clean
git stash push -m "Pre-upgrade changes"  # If needed
...
# Why --allow-dirty?
# - Allows upgrade with uncommitted changes (use with caution)
# - Better: Commit or stash changes first
```

> [!WARNING]
> **Pattern Learned**: `ng update` requires clean git repository. Use `--allow-dirty` only if necessary, but prefer clean state.

**Expected Result**: Build will fail. This is normal.

**Post-Upgrade Verification**:
```bash
# Verify Angular version upgraded
npm list @angular/core --depth=0

# Verify build (may fail - expected)
npm run build

# Check for automated migrations executed
...
> [!CRITICAL]
> **MANDATORY PREREQUISITE**: MDC Migration MUST be completed before v17 upgrade!
> 
> **Verification**:
> ```bash
# Check for MatLegacy imports
grep -r "MatLegacy" src/ | wc -l
# Must be 0 before proceeding to v17!

# Verify MDC migration complete
grep -r "from '@angular/material/legacy" src/
# Should return no results
```

**Pre-Upgrade Checklist**:
- [ ] **MDC Migration Complete** - 0 MatLegacy imports remaining
- [ ] **Git Repository Clean** - Required for `ng update`
- [ ] **Node.js Version** - v18.x or v20.x for Angular 16-17
- [ ] **Backup Created** - Safety backup before upgrade
- [ ] **Current Build Passes** - Verify before upgrade

```bash
# ⚠️ CRITICAL: Verify prerequisites first!
git status  # Must be clean
npm list @angular/core --depth=0  # Verify current version

# Upgrade to v16
ng update @angular/core@16 @angular/cli@16 @angular/material@16 --force --allow-dirty

# Verify v16 upgrade successful
npm list @angular/core --depth=0
npm run build  # May have errors - expected

# Upgrade to v17 (ONLY after MDC migration complete!)
ng update @angular/core@17 @angular/cli@17 @angular/material@17 --force --allow-dirty

# Verify v17 upgrade successful
npm list @angular/core --depth=0
npm run build  # Should pass if MDC migration was complete
```

> [!WARNING]
> **Pattern Learned**: 
> - Always verify git state before `ng update`
> - Use `--allow-dirty` only if necessary
> - Verify Angular version after each upgrade
...
> 👉 **[View Dependency Audit](../setup/dependency-audit.md)**
