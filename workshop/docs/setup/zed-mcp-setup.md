...
- Migration guides (v145→v15→v16→v17→v18→v19→v20)
...
> **Note**: MCP servers are configured under `context_servers` (not `mcp.servers`).  
> **Alternative**: Install MCP servers as **extensions** from the Zed extensions marketplace (easier setup).  
> Access via: Command Palette → `zed: extensions` → Filter by "Context Servers"
...
1. Run `npm run build`
2. Open **Zed's Agent Panel** (Press `Cmd+?` or use Command Palette → `agent: open`)
3. Paste prompt:
   ```
   Using Angular MCP, fix these Angular build errors using Angular v20 best practices.
   
   [paste errors]
   ```
4. AI uses Angular MCP to get exact migration guides
5. Apply fixes
6. Rebuild
...
1. Open **Zed's Agent Panel** (Press `Cmd+?`)
2. Paste prompt:
   ```
